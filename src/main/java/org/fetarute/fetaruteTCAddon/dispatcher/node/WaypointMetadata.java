...
/** Waypoint（包括站咽喉、Depot throat 等）的编码信息。 解析自 SignAction 文本，便于调度层按照运营商/区间/股道规则处理。 */
public record WaypointMetadata(
  ...
  public static WaypointMetadata throat(
    ...
    return new WaypointMetadata(
        operator, station, Optional.empty(), trackNumber, sequence, WaypointKind.STATION_THROAT);
  }

  public static WaypointMetadata depot(
      String operator, String depotId, int trackNumber, String sequence) {
    return new WaypointMetadata(
        operator, depotId, Optional.empty(), trackNumber, sequence, WaypointKind.DEPOT);
  }
}
