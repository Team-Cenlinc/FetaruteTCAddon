 ...
 * <p>目前 provider 仍为占位实现，但暴露 readiness 标记便于命令层做友好提示。</p>
 */
public final class StorageManager {
    ...
    private void logCurrentBackend() {
        if (storageSettings.backend() == ConfigManager.StorageBackend.MYSQL) {
            ...
            logger.debug("存储后端: sqlite file=" + storageSettings.sqliteSettings().file() + " dialect=" + dialect.name());
        }
        if (!isReady()) {
    ...
    private void bootstrapProvider() {
        ...
        this.storageProvider = StorageProviderFactory.create(storageSettings, storageSchema, dialect, logger, dataFolder);
    }
