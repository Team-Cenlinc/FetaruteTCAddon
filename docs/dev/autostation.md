...
方向说明：
- N/E/S/W/NE/NW/SE/SW 需要结合列车朝向（车头面向方向）推导左右门，避免倒车时方向翻转。
- 斜向站台可直接写 NE/NW/SE/SW，避免用 N/E/S/W 近似时在 45 度站台上产生歧义。
- 列车朝向优先使用车体连续水平向量；直线站台仍可回退到 N/E/S/W，斜向站台不会只按 45 度八向处理，30 度、60 度等角度会保留实际投影。
...
- 斜向站台的 N/E/S/W/NE/NW/SE/SW 门侧判定必须由 `doorL/doorR` 附件位置决定：优先使用相对列车横向投影，必要时再用附件世界位置沿门方向投影。若附件位置不可用或无法区分，不会只靠列车向量猜测动画名。门附件局部轴只保留为 `axisCandidate` 诊断，不再作为斜向站台最终判侧来源。
