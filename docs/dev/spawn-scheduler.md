...
每条线路可配置多条可发车的 `OPERATION/RETURN` route，并通过“交路组 + 权重”保证长期比例正确：
...
- 通过 route metadata 配置：
  ...
  - `spawn_group: <string>`：可选交路组标识。建议先显式创建交路组后再绑定；未配置时，系统会按首站/起点（含 DYNAMIC）自动归并。
  - `spawn_group_weight: <int>`：旧版交路组权重，仅在“未配置 group baseline 且需回退 line baseline”时作为兼容逻辑。
- 若同一线路存在多条候选 route，但没有任何 route 配置 `spawn_weight`（也没有显式 `spawn_enabled=true`），为避免误发车将跳过该线路。
- `RETURN` route 默认参与 SpawnPlan（用于 Layover 复用、折返与站牌预测）。

### 显式交路组（Line.metadata）

交路组定义存放在 `Line.metadata.spawn_groups`，推荐使用命令维护，而不是手改 metadata：

- `spawn_groups: [{"name":"<group>","baselineSec":120}, ...]`
...
- 若未配置 `spawn_group`，系统会尝试按首站/起点自动归组，便于“同终点折返 + 回库”在同池调度。
