...
[FTA] SURC/MT lineBaseline=- status=ACTIVE
  ...
  - route=MT-A op=OPERATION group=short-a weight=- start=SURC:D:DEPOT:1 participates=是 reasons=-
  - route=MT-B op=OPERATION group=short-b weight=- start=SURC:D:DEPOT:2 participates=是 reasons=-
```
